...
const ErrorDialog = (props) => {
    ...
    useEffect(() => {
        setStringError(props.error ? (
                (typeof props.error === 'string') ? props.error : JSON.stringify(props.error, null, 2))
            : null);
    }, [props.error]);
    ...
    return <Dialog onClose={props.onClose} aria-labelledby="simple-dialog-title" open={props.error}>
        <DialogTitle id="simple-dialog-title">{props.title}</DialogTitle>
        <DialogContent>
            Something bad happened. The details of the error are below.
            Please copy them and send them to systems support.
            <p/>
            <textarea id='ErrorDialog-error' readOnly style={{
                height: ((height * 14) + 'px'),
                width: (width + 'ex'),
            }}>
                {stringError}
